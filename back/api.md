...
### images
- `GET /articles/:id/images`　**articleに紐づく画像一覧取得**
Paramでarticle idを送り、articleに紐づくImageResponse[]型のJSONを返す。
各画像には、画像名、SeaweedFS上の保存パス、公開URL、Markdown埋め込み用文字列が含まれる。
**動作確認:** `Invoke-RestMethod -Method Get -Uri "http://localhost:8080/articles/1/images"`

- `POST /articles/:id/images?name=:name`　**articleに紐づく画像新規追加**
Paramでarticle idを送り、Query Paramで画像ファイル名を指定する。
Request Bodyにはmultipart/form-data形式で画像ファイルを送る。
画像はSeaweedFSに/article/:id/:nameの形式で保存され、ImageResponse型のJSONを返す。
**動作確認:** `curl.exe -F "file=@.\1.png" "http://localhost:8080/articles/1/images?name=1.png"`

- `DELETE /articles/:id/images/:name`　**articleに紐づく画像削除**
Paramでarticle idと画像ファイル名を送り、SeaweedFS上の画像を削除する。
削除結果を表すJSONを返す。
**動作確認:** `Invoke-RestMethod -Method Delete -Uri "http://localhost:8080/articles/1/images/1.png"`
...
# ImageResponse
    {
    "name": "1.png",
    "path": "/article/1/1.png",
    "url": "http://localhost:8888/article/1/1.png",
    "markdown": "![1](http://localhost:8888/article/1/1.png \"1\")"
    },